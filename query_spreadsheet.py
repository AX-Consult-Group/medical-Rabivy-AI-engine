...
def filter_hcps(state=None, specialty=None, tier=None, targeted=None,
                ...
                min_pa_burden=None, max_pa_burden=None, extra_filters=None,
                sort_by="propensity_score", top=None):
    """Flexible multi-field filter over the HCP table (covers most
    targeting questions).

    extra_filters: optional {column: (min_val, max_val)} dict for any
    numeric column not covered by the named parameters above - the
    generic fallback ask_a_question.py uses for columns like
    days_since_contact or rep_engagement_score that don't have their
    own hand-built natural-language detector.
    ...
    if min_pa_burden is not None:
        sub = sub[sub["pa_burden"] >= min_pa_burden]
    if max_pa_burden is not None:
        sub = sub[sub["pa_burden"] <= max_pa_burden]
    if extra_filters:
        for col, (min_v, max_v) in extra_filters.items():
            if col not in sub.columns:
                continue  # safety net - shouldn't happen, only real columns get detected
...
def format_filter_hcps(data):
    ...
    header = (f"{data['count']} HCPs match (state={f['state']}, specialty={f['specialty']}, "
              ...
              f"pa_burden={f['min_pa_burden']}-{f['max_pa_burden']}"
