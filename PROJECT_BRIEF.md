# Rabivy Commercial AI Engine - Project Brief

## Overview

This document is the original scope and phased build plan for an AI-powered commercial intelligence platform for Rabivy. For how the system actually works today, file by file, see [README.md](README.md).

The system allows AX Pharmaceuticals hypothetical commercial users to ask natural-language questions (e.g. "Who are the top GLP-1 prescribers in NY?") and receive accurate, data-backed answers.
...
| 4 | Retrieval Layer (RAG) | Complete |
| 5 | AI Evaluation Layer | Complete |
| 6 | Conversational Interface | Complete (agent + local chat UI) |

**Figure 1: Project Pipeline** (rough, high-level - shows where the agent adds on top of the original retrieval pipeline; see README.md for the actual, detailed breakdown of every file)
...
2. Passes it to the query router
3. Retrieves the relevant fact(s) from the structured or document engine, or both
...
For the actual, current, file-by-file breakdown of how this was built - including the agentic layer, the evaluation harness, and everything that changed along the way - see [README.md](README.md).